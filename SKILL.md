...
4. Packaging plan: after style confirmation, use upstream `$video-use` to draft a video/subtitle/asset-aware packaging plan.
...
4. 包装方案：确认风格后，使用上游 `$video-use` 基于视频、字幕和素材生成包装方案。
...
7. Use upstream `$video-use` to draft a packaging plan from the video, subtitles, normalized timings, EDL, selected style, and `asset-manifest.json`.
8. Rewrite that plan into a Remotion/React/GSAP implementation prompt and ask the user to confirm it before generating any composition code.
9. Use `references/remotion-style-examples/` only as a read-only reference, then create or adapt the user's Remotion project to the confirmed implementation prompt. Do not register or include the five reference compositions in the user project.
10. Extract keyframes and verify face/mouth/subtitle safety, element overlap, blank frames, and placement before opening Studio.
11. Start Remotion Studio for preview only after QA passes. Do not render a final video by default.
12. Every completion message must include the next-step prompt: ask whether the user wants to review/adjust the cut, continue into packaging animation, or export/finalize.
...
7. 使用上游 `$video-use` 基于视频、字幕、归一化时间、EDL、已选风格和 `asset-manifest.json` 生成包装方案。
8. 把包装方案改写成 Remotion/React/GSAP 可执行 prompt，并在生成任何 composition 代码前请用户确认。
9. 只把 `references/remotion-style-examples/` 当作只读参考，然后基于已确认的实现 prompt 创建或改造用户自己的 Remotion 工程。不要在用户工程里注册或包含 5 个参考 composition。
10. 打开 Studio 前先抽关键帧，检查遮脸/遮嘴/遮字幕、元素重叠、空白帧和错位。
11. QA 通过后再启动 Remotion Studio 让用户预览。默认不要渲染最终视频。
12. 每次完成阶段性产物后，回复里必须给下一步提示：询问用户要预览/调整剪辑、继续包装动效，还是导出/定稿。
