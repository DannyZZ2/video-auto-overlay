...
11. Use `references/remotion-style-examples/` only as a read-only reference, then create or adapt the user's Remotion project to the confirmed implementation prompt. Do not register or include the five reference compositions in the user project.
...
11. 只把 `references/remotion-style-examples/` 当作只读参考，然后基于已确认的实现 prompt 创建或改造用户自己的 Remotion 工程。不要在用户工程里注册或包含 5 个参考 composition。
