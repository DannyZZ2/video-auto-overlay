...
description: Orchestrate raw or prepared video into word-timed Remotion overlay animations by using the upstream video-use skill for optional fine-cut editing, word-level transcription, packaging-plan drafting from video/subtitles/assets/keyword-effect references/diagnostic HUD packaging patterns/visual style snapshots, normalizing video-use outputs, collecting current-project assets by filename-only subtitle keyword matching, rewriting the confirmed plan into a Remotion/React/GSAP implementation prompt, and using five bundled Remotion examples plus still snapshots as read-only style references. Use when the user asks for AI video packaging, Remotion overlay animation, keyword animation effects, diagnostic HUD cards, evidence-driven talking-head overlays, subtitle-timed motion graphics, talking-head enhancement, fine-cut talking-head cleanup, project-asset-aware video packaging, or a repeatable workflow from video-use transcript/EDL to Remotion Studio preview with pre-Studio keyframe QA.
---
...
4. Packaging plan: after style confirmation, read the keyword-effect library and diagnostic HUD packaging patterns, then use upstream `$video-use` to draft a video/subtitle/asset-aware packaging plan.
5. Implementation prompt: rewrite the user-confirmed packaging plan into an executable Remotion/React/GSAP prompt.
6. Remotion packaging and QA: use the bundled five-example Remotion project and still snapshots as read-only style references, create or adapt a separate user Remotion project with only the user's composition(s), extract keyframes, and pass QA before Studio preview.

1. 对话编排：询问用户是否需要剪辑。
2. 归一化：把 `video-use` 产物转换成稳定 JSON 契约。
3. 素材清单：收集当前项目素材，并只用文件名匹配字幕关键词。
4. 包装方案：确认风格后，读取关键词动效库和诊断 HUD 包装模式，并使用上游 `$video-use` 基于视频、字幕和素材生成包装方案。
5. 实现 prompt：把用户确认后的包装方案改写成 Remotion/React/GSAP 可执行 prompt。
6. Remotion 包装与 QA：把内置 5 个案例工程和静态样张作为只读风格参考，创建或改造只包含用户 composition 的独立 Remotion 工程，抽关键帧并通过 QA 后再开 Studio 预览。
...
6. Read `references/remotion-style-guide.md`, inspect the relevant still under `references/style-snapshots/`, confirm the visual style, and default to `DarkDiagnosticHUDTest` (`dark-diagnostic-hud`) unless the user explicitly asks for another style.
7. Read `references/diagnostic-hud-packaging-patterns.md` before drafting the packaging plan; use it as the default card/text/evidence pattern library for `dark-diagnostic-hud`.
8. Read `references/keyword-animation-effects.md` before drafting the packaging plan; use it as the allowed/default keyword-effect library for spoken payoff words and local interaction beats.
9. Use upstream `$video-use` to draft a packaging plan from the video, subtitles, normalized timings, EDL, selected style, diagnostic packaging patterns, keyword-effect library, and `asset-manifest.json`.
10. Rewrite that plan into a Remotion/React/GSAP implementation prompt and ask the user to confirm it before generating any composition code.
11. Use `references/remotion-style-examples/` and `references/style-snapshots/` only as read-only references, then create or adapt the user's Remotion project to the confirmed implementation prompt. Do not register or include the five reference compositions or snapshot images in the user project.
12. Extract keyframes and verify face/mouth/subtitle safety, element overlap, blank frames, and placement before opening Studio.
13. Start Remotion Studio for preview only after QA passes. Do not render a final video by default.
14. Every completion message must include the next-step prompt: ask whether the user wants to review/adjust the cut, continue into packaging animation, or export/finalize.
...
6. 读取 `references/remotion-style-guide.md`，查看 `references/style-snapshots/` 中对应静态样张，先确认视觉风格；除非用户明确指定其他风格，否则默认使用 `DarkDiagnosticHUDTest`（`dark-diagnostic-hud`）。
7. 生成包装方案前读取 `references/diagnostic-hud-packaging-patterns.md`；把它作为 `dark-diagnostic-hud` 的默认卡片、文字和证据模式库。
8. 生成包装方案前读取 `references/keyword-animation-effects.md`；把它作为口播重点词和局部交互节点的默认/允许关键词动效库。
9. 使用上游 `$video-use` 基于视频、字幕、归一化时间、EDL、已选风格、诊断包装模式、关键词动效库和 `asset-manifest.json` 生成包装方案。
10. 把包装方案改写成 Remotion/React/GSAP 可执行 prompt，并在生成任何 composition 代码前请用户确认。
11. 只把 `references/remotion-style-examples/` 和 `references/style-snapshots/` 当作只读参考，然后基于已确认的实现 prompt 创建或改造用户自己的 Remotion 工程。不要在用户工程里注册或包含 5 个参考 composition 或样张图片。
12. 打开 Studio 前先抽关键帧，检查遮脸/遮嘴/遮字幕、元素重叠、空白帧和错位。
13. QA 通过后再启动 Remotion Studio 让用户预览。默认不要渲染最终视频。
14. 每次完成阶段性产物后，回复里必须给下一步提示：询问用户要预览/调整剪辑、继续包装动效，还是导出/定稿。
...
- `references/style-snapshots/`: still visual snapshots rendered from the five reference compositions for fast style inspection.
- `references/diagnostic-hud-packaging-patterns.md`: reusable card/text/evidence generation rules distilled from the diagnostic HUD reference video.
...
- `references/style-snapshots/`：从 5 个参考 composition 抽出的静态视觉样张，用于快速确认风格。
- `references/diagnostic-hud-packaging-patterns.md`：从诊断 HUD 参考视频提炼出的卡片、文字和证据生成规则。
...
- Do not copy `references/style-snapshots/` into the user's generated Remotion project. They are visual references only.
...
- Treat `references/diagnostic-hud-packaging-patterns.md` as a packaging-pattern library, not proof. Do not fabricate screenshots, chat records, repository names, scores, or source evidence.
...
- 不要把 `references/style-snapshots/` 复制进用户生成的 Remotion 工程。它们只是视觉参考。
- 默认保持底色视频画面不变。只叠加弹出卡片；除非用户明确要求，不添加全局进度条、背景处理、扫描网格、压暗蒙层、背景图、视频滤镜或亮度变化。
- 把 `references/diagnostic-hud-packaging-patterns.md` 当作包装模式库，而不是证据来源。不要伪造截图、聊天记录、仓库名、分数或来源证据。
